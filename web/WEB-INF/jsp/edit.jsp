        ...
        <c:forEach var="type" items="<%=SectionType.values()%>">
            <c:choose>
                ...
                <c:when test="${type eq 'EXPERIENCE' || type eq 'EDUCATION'}">
                    <h4>${type.title}</h4>
                    <c:forEach var="org" items="${resume.getSection(type).organizations}" varStatus="count">
                        <dl>
                            <dd><input type="text" name='${type}' placeholder="Название" size=100
                                       value="${org.homePage.name}"></dd>
                        </dl>
                        <dl>
                            <dd><input type="text" name='${type}url' placeholder="Сайт" size=100
                        ...
                        <div>
                            <c:forEach var="position" items="${org.positions}">
                                ...
                                <dl>
                                    <dd>
                                        <input type="text" name="${type}${count.index}startDate"
                                               placeholder="Начало: MM/yyyy" size=20
                                               value="<%=DateUtil.getStringDate(position.getStartDate())%>">
                                    </dd>
                                </dl>
                                <dl>
                                    <dd>
                                        <input type="text" name="${type}${count.index}endDate"
                                               placeholder="Окончание: MM/yyyy" size=20
                                               value="<%=DateUtil.getStringDate(position.getEndDate())%>">
                                </dl>
                                <dl>

                                    <dd><input type="text" name="${type}${count.index}title" placeholder="Заголовок"
                                               size=55
